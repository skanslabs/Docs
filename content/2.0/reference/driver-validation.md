...
*Table + counts generated 2026-07-17 16:42 UTC from the validation ledger via `SkansCP --driver-validation` — never hand-edited.*

**Cert-deploy (120 drivers):** 8 hardware-validated · 9 emulation-validated · 1 known-issue · **102 spec-verified (pending a device)**. The monitor columns (SNMP/syslog) are marked only where a real device proved them.
...
| `cisco9200` | Cisco Catalyst 9200 | 🧪 emu | 📋 spec | 🧪 emu |  | 🧪 emu | 🧪 emu | 📋 spec |  |
| `cisco9300` | Cisco Catalyst 9300 | 🧪 emu | 📋 spec | 🧪 emu |  | 🧪 emu | 🧪 emu | 📋 spec |  |
| `cisco9500` | Cisco Catalyst 9500 | 🧪 emu | 📋 spec | 🧪 emu |  | 🧪 emu | 🧪 emu | 🧪 emu |  |
| `ciscoasa` | Cisco ASA | 🧪 emu | 📋 spec | 📋 spec |  | 🧪 emu | ⚠️ issue | 📋 spec |  |
| `ciscoftd` | Cisco FTD | 🧪 emu |  |  |  |  |  |  |  |
...
| `ciscowlc` | Cisco WLC | 🧪 emu | 📋 spec |  |  |  | 🧪 emu | 🧪 emu | 🧪 emu |
...
- **`cisco9200` · cert-deploy / config-backup** — emulation on Catalyst 9000v UADP 17.18 (CML): served serial == issued serial (cert); full running-config captured by `--backup-configs` (57 KB) — not a hostname grep.
- **`cisco9300` · cert-deploy / config-backup** — emulation on Catalyst 9000v UADP (CML): cert serial match; full running-config captured (39 KB).
- **`cisco9500` · cert-deploy / config-backup / snmp** — emulation on Catalyst 9000v Q200 (CML): cert serial match; full running-config (34 KB); SNMP poll proven — 201 `iface.oper` metric docs in skans-metrics via DNAT UDP-161.
- **`ciscowlc` · cert-deploy / config-backup / snmp / syslog** — emulation on Catalyst 9800-CL 17.18 (CML): cert on :443 web-admin; 22 KB running-config with **SET-GOLDEN + DRIFT proven in the console Config tab** (real change → DRIFT badge → re-golden → in-sync); SNMP poll (43 docs) + syslog & SNMP traps (483 skans-netlog docs, rendered in Logs). LAN-bridged so it can reach .92.
- **`ciscoasa` · cert-deploy** — emulation on ASAv 9.24 (CML): PKCS#12 trustpoint bound to HTTPS, on-wire self-verify.
- **`ciscoasa` · config-backup** — known-issue on ASAv 9.24 (CML): enable-mode fix reaches priv-exec + reads real config; the emulated ASAv stalls its SSH stream under Renci.SshNet after ~10 lines (openssh streams the full 900-line config in 2.6s) — an emulation/library quirk, not a logic bug.
- **`ciscoiosxe` · cert-deploy** — emulation on Cat8000v / CSR1000v / IOL-XE (CML): PKCS#12 trustpoint import across 3 router platforms.
- **`cisconxos` · cert-deploy** — emulation on Nexus 9300v NX-OS 10.6 (CML): NX-API :443 serves Skans Root CA, serial match.

- **`ciscoftd` · cert-deploy** — emulation on Cisco FTDv (FDM 10.0, CML): FDM REST cert-deploy proven — the management HTTPS listener serves the Skans Root CA leaf, served serial == issued. 3 driver bugs found+fixed (upsert 422-not-400, issuing-CA upload, `WebUICertificate.trustChain`) + FDM initial-provision via REST.
